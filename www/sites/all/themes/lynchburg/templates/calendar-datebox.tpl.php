 ...
 */
if (!empty($calendar_tooltips_text)) {
  $calendar_tooltips_text = preg_replace('#</div>$#i', '', $calendar_tooltips_text);
  $calendar_tooltips_text .= '<br /><a href="/events">view full calendar</a></div>';
}
?>
<div class="<?php print $granularity ?> <?php print $class; ?>"> 
  <?php print !empty($selected) ? '<a href="/events/' . $date . '--' . $date . '">' . $day . '</a>' . $calendar_tooltips_text : $day; ?> 
</div>
